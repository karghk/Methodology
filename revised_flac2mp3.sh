...
bar(){ local pct=$1 w=$2 body filled empty; ((pct<0))&&pct=0; ((pct>100))&&pct=100; body=$((w-4)); ((body<1))&&body=1; filled=$((body*pct/100)); empty=$((body-filled)); printf '%*s' "$filled" ''|tr ' ' '#'; printf '%*s' "$empty" ''|tr ' ' '-'; printf '%3d%%' "$pct"; }
summary_bar(){ local pct=$1 w=$2 filled empty; ((pct<0))&&pct=0; ((pct>100))&&pct=100; filled=$((w*pct/100)); empty=$((w-filled)); printf '%*s' "$filled" ''|tr ' ' '#'; printf '%*s' "$empty" ''|tr ' ' '.'; }
...
frame_bar(){ local p=$1; printf '\033[2K'; bar "$p" "$WIDTH"; printf '\n'; }
...
greeting(){ clear_screen; WIDTH="$(terminal_width)"; printf 'Source      : %s\n' "$SOURCE_ROOT"; printf 'Destination : %s\n\n' "$DEST_ROOT"; printf 'Artist:\n'; }
scan_frame(){ local artist=$1 total=$2 albums=$3 current=$4; frame_begin; frame_line "Artist: $artist"; frame_line ''; frame_line 'Scanning...'; frame_line "$(fit "$current")"; frame_line ''; frame_line "Found: $total tracks"; frame_line "       $albums albums"; }
...
convert_frame(){
 ...
 frame_line "Album:  $an/$at    Track: $tn/$tt"
 frame_line "File:   $(fit "$file")"
 frame_bar "$pct"
 frame_line 'Encoding...'
...
summary_frame(){ local artist=$1 elapsed=$2 done=$3 total=$4 failed=$5 skipped=$6 embedded=$7 copied=$8 extras=$9 src=${10} dst=${11} reduced=${12} pct=${13}; clear_screen; WIDTH="$(terminal_width)"; printf 'Artist: %s\n' "$artist"; printf '\n%s elapsed\n' "$elapsed"; printf '> 100%% [%d/%d]\n' "$done" "$total"; printf '> [%d] failed ; [%d] skipped\n' "$failed" "$skipped"; printf '> [%d/%d] images embedded\n' "$embedded" "$total"; printf '> [%d/%d] extra files copied\n\n' "$copied" "$extras"; printf '[%s] Source FLACs + extras\n' "$src"; summary_bar 100 "$WIDTH"; printf '\n\n[%s] Converted MP3s + extras\n' "$dst"; summary_bar "$pct" "$WIDTH"; printf '\n\n> %s reduced\n\n' "$reduced"; printf '%s\n\n' '----------------------------------------'; printf '[C] Convert another artist\n[Q] Quit\n'; [[ -s "$LAST_ERROR_LOG" ]]&&printf '[V] View error log\n'; }

convert_track(){
 local src=$1 dst=$2 dur_us=$3 artist=$4 an=$5 at=$6 tn=$7 tt=$8 before=$9 start=${10} log=${11}
 local efile line out_us=0 pct=0 overall=0 bitrate='--' size dur name elapsed eta done_units status
 efile=$(mktemp); dur=$(hdur "$(duration "$src")"); name=${src##*/}; mkdir -p "${dst%/*}"
 ffmpeg -hide_banner -loglevel error -nostats -nostdin -y -i "$src" -map 0:a:0 -map 0:v? -map_metadata 0 -c:a libmp3lame -q:a 2 -c:v mjpeg -id3v2_version 3 -write_id3v1 1 -progress pipe:1 "$dst" 2>"$efile" |
 while IFS= read -r line; do
   case "$line" in
     out_time_us=*) out_us=${line#*=};;
     out_time_ms=*) [[ "$out_us" == 0 ]]&&out_us=$(( ${line#*=} * 1000 ));;
     bitrate=*) bitrate=${line#*=};;
     progress=end) out_us=$dur_us; pct=100;;
   esac
   if [[ "$line" == out_time_us=* || "$line" == out_time_ms=* || "$line" == progress=end ]]; then
     [[ "$out_us" =~ ^[0-9]+$ ]] || out_us=0
     if ((dur_us>0)); then pct=$((out_us*100/dur_us)); else pct=0; fi; ((pct>100))&&pct=100
     overall=$(( (before*100+pct)/tt )); ((overall>100))&&overall=100
     size=$(hsize "$(size_of "$dst")"); elapsed=$(( $(date +%s)-start )); done_units=$((before*100+pct)); eta=0; ((done_units>0))&&eta=$((elapsed*(tt*100-done_units)/done_units))
...
run_artist(){
 ...
 mkdir -p "$out"; while IFS= read -r -d '' file; do tracks+=("$file"); done < <(find "$dir" -type f -iname '*.flac' -print0|sort -z); total=${#tracks[@]}; ((total>0))||{ clear_screen; printf 'Artist: %s\n\nNo FLAC tracks found.\n' "$artist"; return; }
...
main(){ hide_cursor; while true; do greeting; IFS= read -r artist; [[ -z "$artist" ]]&&continue; if ! dir=$(find_artist "$artist"); then clear_screen; printf 'Artist:\n\n%s not found.\n\nPress Enter to continue.' "$artist"; read -r _; continue; fi; run_artist "$artist" "$dir"; while true; do IFS= read -r -n1 choice; printf '\n'; case "${choice^^}" in C) break;; Q|'') return;; V) clear_screen; printf 'Error Log\n\n'; cat "$LAST_ERROR_LOG"; printf '\nPress Enter to return.'; read -r _;; esac; done; done; }
main "$@"
