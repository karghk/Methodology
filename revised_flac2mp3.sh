...
bar(){ local pct=$1 width=$2 inner filled empty; ((pct<0))&&pct=0; ((pct>100))&&pct=100; inner=$((width-5)); ((inner<1))&&inner=1; filled=$((inner*pct/100)); empty=$((inner-filled)); printf '%*s' "$filled" ''|tr ' ' '█'; printf '%*s' "$empty" ''|tr ' ' '░'; printf '%3d%%' "$pct"; }
...
conversion_static(){ local artist=${1-} an=${2-} at=${3-} tn=${4-} tt=${5-0} file=${6-} dur=${7-}; put "$ARTIST_ROW" "Artist: $artist > Album $an/$at > Track $tn/$tt >"; put 5 "$(fit "$file")"; put 6 ''; put 7 ''; put 8 'Encoding...'; put 9 'Bitrate: '; put 10 'Size: '; put 11 "Track Duration:  $dur"; put 12 'Elapsed: '; put 13 'ETA: '; put 14 ''; put 15 ''; put 16 ''; put 17 '0% Complete'; }
conversion_dynamic(){ local pct=${1:-0} overall=${2:-0} bitrate=${3:---} size=${4:---} elapsed=${5:---} eta=${6:---}; clr "$BAR_ROW"; move "$BAR_ROW" 1; bar "$pct" "$WIDTH"; val "$BITRATE_ROW" 18 24 "$bitrate"; val "$SIZE_ROW" 18 32 "$size"; val "$ELAPSED_ROW" 10 12 "$elapsed"; val "$ETA_ROW" 6 12 "$eta"; clr "$OVERALL_BAR_ROW"; move "$OVERALL_BAR_ROW" 1; summary_bar "$overall" "$WIDTH"; val "$OVERALL_TEXT_ROW" 1 3 "$overall"; move "$OVERALL_TEXT_ROW" 4; printf '%% Complete'; }
...
convert_track(){
 ...
 local pfile efile pid status pct overall bitrate size elapsed eta current_bytes total_bytes out_us time_pct dur name
 ...
 while :; do
  ...
  bitrate=$(awk -F= '/^bitrate=/{v=$2} END{print v}' "$pfile" 2>/dev/null); [[ $bitrate =~ ^[0-9]+(\.[0-9]+)?kbits/s$ ]]||bitrate='256 kb/s'
