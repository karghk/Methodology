...
readable_bar(){ local pct=$1 width=$2 inner filled empty; ((pct<0))&&pct=0; ((pct>100))&&pct=100; inner=$((width-9)); ((inner<3))&&inner=3; filled=$((inner*pct/100)); empty=$((inner-filled)); printf '['; printf '%*s' "$filled" ''|tr ' ' '#'; printf '%*s' "$empty" ''|tr ' ' '-'; printf '] %3d%%' "$pct"; }
...
write_at(){ local row=${1-1} text=${2-}; clear_row "$row"; move_to "$row" 1; printf '%s' "$text"; }
...
greeting(){ clear_screen; WIDTH="$(terminal_width)"; printf 'Source      : %s\n' "$SOURCE_ROOT"; printf 'Destination : %s\n' "$DEST_ROOT"; printf '\n\n\n' ; printf 'Artist:\n'; }
scan_frame(){ local artist=${1-} total=${2-0} albums=${3-0} current=${4-}; write_at "$ARTIST_ROW" "Artist: $artist"; write_at 7 ''; write_at 8 'Scanning...'; write_at 9 "$(fit "$current")"; write_at 10 ''; write_at 11 "Found: $total tracks"; write_at 12 "       $albums albums"; }
conversion_static(){ local artist=${1-} an=${2-} at=${3-} tn=${4-} tt=${5-} file=${6-} dur=${7-}; WIDTH="$(terminal_width)"; write_at "$ARTIST_ROW" "Artist: $artist > Album $an/$at > Track $tn/$tt >"; write_at 7 "$(fit "$file")"; write_at 9 'Encoding...'; write_at 12 "Track Duration:  $dur"; write_at 15 ''; }
conversion_dynamic(){ local pct=${1-0} overall=${2-0} bitrate=${3---} size=${4---} elapsed=${5---} eta=${6---}; write_bar "$CONVERSION_BAR_ROW" "$pct"; write_at "$BITRATE_ROW" "Bitrate:         $bitrate"; write_at "$SIZE_ROW" "Size:            $size"; write_at "$ELAPSED_ROW" "Elapsed:         $elapsed"; write_at "$ETA_ROW" "ETA:             $eta"; write_bar "$OVERALL_BAR_ROW" "$overall"; write_at "$OVERALL_TEXT_ROW" "$overall% Complete"; }
summary_frame(){ local artist=${1-} elapsed=${2-} done=${3-0} total=${4-0} failed=${5-0} skipped=${6-0} embedded=${7-0} copied=${8-0} extras=${9-0} src=${10-} dst=${11-} reduced=${12-} pct=${13-0}; WIDTH="$(terminal_width)"; write_at "$ARTIST_ROW" "Artist: $artist"; write_at 7 ''; write_at 8 "$elapsed elapsed"; write_at 9 "> 100% [$done/$total]"; write_at 10 "> [$failed] failed ; [$skipped] skipped"; write_at 11 "> [$embedded/$total] images embedded"; write_at 12 "> [$copied/$extras] extra files copied"; write_at 13 ''; write_at 14 "[$src] Source FLACs + extras"; clear_row 15; move_to 15 1; summary_bar 100 "$WIDTH"; write_at 16 "[$dst] Converted MP3s + extras"; clear_row 17; move_to 17 1; summary_bar "$pct" "$WIDTH"; write_at 18 "> $reduced reduced"; write_at 20 '----------------------------------------'; write_at 22 '[C] Convert another artist'; write_at 23 '[Q] Quit'; if [[ -s "$LAST_ERROR_LOG" ]]; then write_at 24 '[V] View error log'; else clear_row 24; fi; }
convert_track(){
 local src=${1-} dst=${2-} dur_us=${3-0} artist=${4-} an=${5-} at=${6-} tn=${7-} tt=${8-0} before=${9-0} start=${10-0} log=${11-}
 local pfile efile pid status out_us pct overall bitrate size elapsed eta done_units dur name current_bytes total_bytes time_pct
 ...
 pid=$!; conversion_static "$artist" "$an" "$at" "$tn" "$tt" "$name" "$dur"
 while :; do
   ...
   if ((current_bytes>0 && out_us>0 && dur_us>0 && pct==0)); then time_pct=$((out_us*100/dur_us)); ((time_pct>99))&&time_pct=99; pct=$time_pct; fi
   ...
   size="$(hsize "$current_bytes") / $(hsize "$total_bytes")"; elapsed=$(( $(date +%s)-start )); done_units=$((before*100+pct)); eta=0; ((done_units>0))&&eta=$((elapsed*(tt*100-done_units)/done_units))
   conversion_dynamic "$pct" "$overall" "$bitrate" "$size" "$(hdur "$elapsed")" "$(hdur "$eta")"
   if ! kill -0 "$pid" 2>/dev/null; then wait "$pid"; status=$?; break; fi
   sleep .20
 done
 if ((status==0)); then pct=100; overall=$(( (before*100+100)/tt )); ((overall>100))&&overall=100; bitrate=$(awk -F= '/^bitrate=/{v=$2} END{print v}' "$pfile" 2>/dev/null); [[ -n "$bitrate" ]]||bitrate='256 kb/s'; current_bytes=$(size_of "$dst"); size="$(hsize "$current_bytes") / $(hsize "$total_bytes")"; elapsed=$(( $(date +%s)-start )); conversion_dynamic 100 "$overall" "$bitrate" "$size" "$(hdur "$elapsed")" '00m00s'; else printf 'FILE: %s\n' "$src" >>"$log"; cat "$efile" >>"$log"; printf '\n' >>"$log"; rm -f "$dst"; fi
...
run_artist(){
 ...
 LAST_ERROR_LOG="$out/.flac2mp3-errors.log"; : >"$LAST_ERROR_LOG"; for file in "${tracks[@]}"; do scan_frame "$artist" "$total" "$album_total" "${file##*/}"; sleep .02; done
 start=$(date +%s)
