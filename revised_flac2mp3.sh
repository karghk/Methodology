...
for cmd in ffmpeg ffprobe find sort awk mktemp stat date tput; do
...
convert_track(){
    ...
    if ((status==0)); then
        current_bytes=$(size_of "$dst"); elapsed=$(( $(date +%s)-start ))
        conversion_dynamic 100 "$(( (before*100+100)/tt ))" 256000000 "$(hsize "$current_bytes") / $(hsize "$total_bytes")" "$(hdur "$elapsed")" '00m00s' "$dur"
    else
...
run_artist(){
    ...
    for ((i=0;i<total;i++)); do
        file=${tracks[$i]}; rel=${file#"$dir"/}; album=${rel%/*}; [[ "$album" == "$rel" ]]&&album=Singles
        an=${album_no[$album]}; dst="$out/${rel%.*}.mp3"; mkdir -p "${dst%/*}"
    ...
    ((src_bytes>0))&&pct=$((dst_bytes*100/src_bytes))||pct=0; ((pct>100))&&pct=100; reduced=$((src_bytes-dst_bytes)); ((reduced<0))&&reduced=0
    elapsed=$(( $(date +%s)-start )); summary_frame "$artist" "$(hdur "$elapsed")" "$((done+skipped))" "$total" "$failed" "$skipped" "$embedded" "$copied" "${#extras[@]}" "$(hsize "$src_bytes")" "$(hsize "$dst_bytes")" "$(hsize "$reduced")" "$pct"
}
